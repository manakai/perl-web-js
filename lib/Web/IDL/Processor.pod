...
An annotated type with the [AllowShared] extended attributes.  The
C<1>st item is a type value representing the type without annotation.

=item Clamp

An annotated type with the [Clamp] extended attributes.  The C<1>st
item is a type value representing the type without annotation.

=item EnforceRange

An annotated type with the [EnforceRange] extended attributes.  The
C<1>st item is a type value representing the type without annotation.
...
=item TreatNullAsEmptyString

An annotated type with the [TreatNullAs=EmptyString] extended
attributes.  The C<1>st item is a type value representing the type
without annotation.
...
=item ArrayClass (boolean)

Whether the interface is annotated with the [LegacyArrayClass]
extended attribute or not.
...
If the value is C<replaceable>, the setter of the attribute is the Web
IDL [Replaceable] setter.

If the value is C<LenientSetter>, the setter of the attribute is the
Web IDL [LenientSetter] setter.

=item CEReactions [boolean]

Whether the [CEReactions] extended attribute is in effect or not.

=item SecureContext (boolean)

Whether the object is annotated (directly or indirectly) with the
[SecureContext] extended attribute or not.

=item TreatNonObjectAsNull (boolean)

Whether the callback is annotated with the [TreatNonObjectAsNull]
extended attribute or not.
